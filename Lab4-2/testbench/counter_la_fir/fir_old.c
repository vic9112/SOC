...
void __attribute__ ( ( section ( ".mprjram" ) ) ) initfir() {
	...
	// program tap length
	wb_write(reg_fir_tap, tap_length);

	// program coefficient
	for (uint32_t i = 0; i < tap_length; i++) {
...
/*
	...
	while (t < N-1) {		
		...
		outputsignal[t-1] = wb_read(reg_fir_y_out);  // read Y from fir
...
*/
    ...
    uint8_t register t = 1;

	wb_write(reg_fir_x_in, 0);
